...
def sphericalToCartesianPointCloud(sphericalPointCloud):#计算点云的笛卡尔坐标
...
def parseTrackTLV(tlvData, tlvLength):
    ...
    targets = np.empty((numDetectedTargets,16))#16列
...
#         return 0, np.empty((0, 16))
#
#     # Create a buffer for the target data
#     targetDataBuffer = np.frombuffer(tlvData, dtype=np.float32)从原始数据直接创建numpy数组避免逐个解析
#
#     # Extract the relevant data
#     targetData = targetDataBuffer.reshape((numDetectedTargets, 28))  # 1 uint32 and 27 float32
#                                    使用reshape一次性提取所有目标数据而不在循环逐个提取
#     targets = np.empty((numDetectedTargets, 16))
#     targets[:, 0] = targetData[:, 0]  # Target ID
#     targets[:, 1] = targetData[:, 1]  # X Position
#     targets[:, 2] = targetData[:, 2]  # Y Position
#     targets[:, 3] = targetData[:, 3]  # Z Position
#     targets[:, 4] = targetData[:, 4]  # X Velocity
#     targets[:, 5] = targetData[:, 5]  # Y Velocity
#     targets[:, 6] = targetData[:, 6]  # Z Velocity
#     targets[:, 7] = targetData[:, 7]  # X Acceleration
#     targets[:, 8] = targetData[:, 8]  # Y Acceleration
#     targets[:, 9] = targetData[:, 9]  # Z Acceleration
#     targets[:, 10] = targetData[:, 26]  # G
#     targets[:, 11] = targetData[:, 27]  # Confidence Level
#
#     return numDetectedTargets, targets
