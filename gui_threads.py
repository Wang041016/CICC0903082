...
class parseUartThread(QThread):
        ...
        def run(self):
                ...
                self.fin.emit(outputDict)#连接到 self.fin信号的槽函数都会接收到 outputDict 作为输入
...
class updateQTTargetThread3D(QThread):
    ...
    #     # Get necessary track data  接受tlv解析后的坐标targetdata数据实现可视化
    #     tid = int(track[0])
    #     x = track[1]
    #     y = track[2]
    #     z = track[3]
    ...
    #     track.setVisible(True)
        ...
        # track = self.ellipsoids[tid]
        # mesh = getBoxLinesCoords(x,y,z)#生成三d包围框 之后在u getboxlinecood以线条方式显示该范围 最后在run更新
        # track.setData(pos=mesh,color=trackColor,width=2,antialias=True,mode='lines')
        # track.setVisible(True)
    ...
    def run(self):
        ...
        # Create a list of just X, Y, Z values to be plotted toPlot即为点云的坐标数据
